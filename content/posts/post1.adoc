...
TIP: If you want the outpu to look familiar, copy (or link) the AsciiDoc stylesheet, asciidoc.css, to the output directory. Really?
...
This {plus} sign is produced with Asciidoc macros.

.Inline markup
* underscores around a phrase place _emphasis_
...
* +++//+++ surrounded by two empty lines separates two adjacent lists. The follow is another list:

// separate two adjacent lists using a line comment (only the leading // is required)

* handle words with unicode characters: Romé, ℵ, ψ
* special attributes:
...
** characters can be escaped using a +++{backslash}+++ {backslash}
** for instance, you can escape a quote inside emphasized text like 'Here\'s Johnny!'
** you can safely use reserved XML characters like <, > and &, which are escaped without special characters
...
You can write text http://example.com[with inline links], optionally{sp}using an explicit
link:http://example.com[link prefix].
In either case, the link can have a http://example.com?foo=bar&lang=en[query string].
Note that there seems to require escape characters when underscores appear in the URL. But how?




== Math
Inline math content: latexmath:[C = \alpha\psi + \beta Y^{\gamma} + \epsilon].
The following is a display math:
[stem]
++++
\def\f#1{f(#1)}
...
== Lists Upon Lists
dot . creates a title for the following block.

.Adjacent lists
* this list should join
...
. These items
+
Can be joined by another paragraph with a {plus} sign.
...
+
another paragraph for the definition of term 1
...
Here's another parameterized quote:
...
.source code listing
The following is a Ruby cource code:
[source, ruby]
----
